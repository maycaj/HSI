...
def plotPCA(components, bands2ignore, title): # plot the PCA components 
    """
    Plots the PCA components.

    Args:
        components: Array containing the PCA components.
        bands2ignore: Number of bands or list of indices of bands to ignore.
    ...
    wavelengths = wavelengths[bands2ignore:] # remove early bands from training
    for i in range(components.shape[0]):
        plt.scatter(wavelengths,components[i], label=f'Component: {i}')
        if i > 2: # don't plot more than 3 dimensions
            break
    plt.legend()
    plt.title(title)
    plt.xlabel('Band (nm)')
...
for n_PCA in n_PCAs:
    ...
    for selectedNum in selectedNums:
        for i in range(iterations): # iterate multiple times for error bars
            ...
            pca = make_pipeline(StandardScaler(), PCA(n_components=n_PCA, random_state=random_state))
            n_neighbors = 5
            ...
            for i, testID in enumerate(testIDs): # loop over each patient
                testID = testID.item()
