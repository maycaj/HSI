...
def chrom_dot_spectra(data, nmStartEnd, spectraName, outputName, absorbPath, d60=True, normalized=True, plot=True):
    ...
    # matchedWave = pd.DataFrame({'AbsorbWaves': absorbWaves, 'SkinWaves': skinWaves_float})

    # # remove the Skin Wave Values that are outside of the range of the absorbance values
    # maxSkin = float('inf')
    # minSkin = float('inf')
    # for wavelength in skinWaves_float: # find the maximum and minimum wavelengths in the skin waves
...
if __name__ == '__main__':
    ...
    ## Load in Data
    # csv_path = '/Users/maycaj/Documents/HSI/PatchCSVs/May_29_NOCR_FullRound1and2AllWLs_medians.csv' 
